...
if ! screen -list | grep -q "$MONITOR_SCREEN_NAME"; then
    ...
    # Export all necessary variables so they're available to the screen session
    export LOG_FILE RESTART_LOG DISCORD_WEBHOOK SCREEN_NAME
    export WORLD_PATH CRASHED_WORLDS BACKUP_WORLDS ADMIN_LOG
...
is_player_whitelisted() {
    ...
    return 1  # false, player is not whitelisted
}
...
handle_world_crash() {
    ...
    if [ -d "$WORLD_PATH" ]; then
        ...
        log_monitor "Moved crashed world to: $CRASHED_WORLDS/world$timestamp"
    fi
    ...
    if [ -n "$latest_backup" ]; then
        ...
        log_monitor "Restored backup: $latest_backup"
        return 0
    else
        log_admin "ERROR: No backup found to restore!"
        log_monitor "ERROR: No backup found to restore!"
        return 1
    fi
}

echo "$(date '+%Y-%m-%d %H:%M:%S') - Starting monitor..."
echo "$(date '+%Y-%m-%d %H:%M:%S') - Monitoring log file..."
...
tail -F -n0 "$LOG_FILE" | while read -r line; do
    ...
    if [[ "$line" == *"left the game"* ]]; then
        ...
        # Always log to monitor
        log_monitor "Player left: $player_name"
    ...
    if [[ "$line" == *"[Server Watchdog/FATAL]: Considering it to be crashed"* ]]; then
        ...
        if [ "$restart_count" -lt 3 ]; then
            ...
            log_monitor "Server restarted after crash (Attempt $restart_count)"
        else
            curl -H "Content-Type: application/json" \
                 -X POST \
            ...
            log_monitor "Maximum daily restart limit reached after crash"
        fi
    fi
done
EOF
